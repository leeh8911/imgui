...
IMGUI_API bool BeginTooltip(); // begin/append a tooltip window. to create full-featured tooltip (with any kind of
                               // items).
...
struct ImGuiStyle
{
    ...
    float LayoutAlign;        // Element alignment inside horizontal and vertical layouts (0.0f - left/top, 1.0f -
                              // right/bottom, 0.5f - center).
...
struct ImGuiIO
{
    ...
    // all of them.
    // - consider using e.g. Win32's IsDebuggerPresent() as an additional filter (or see ImOsIsDebuggerPresent() in
